    ...
    public void OnMouseDown()
    {
        ...
        if (gameObject.GetComponent<SpriteRenderer>().sprite == AllGameObject.Pipa)
        {
           /* if (AllGameObject.SegedUpgradeTower.GetComponentInParent<TowerShot>())
            {
                AllGameObject.MainCamera.GetComponent<Zoomer>().ChangeScaleObject.Remove(AllGameObject.SegedUpgradeTower.GetComponent<TowerShot>().UpgradeArrowTowerScriptCIrcle);
            }
            else
            if (AllGameObject.SegedUpgradeTower.GetComponentInParent<TowerShot>())
            {
                AllGameObject.MainCamera.GetComponent<Zoomer>().ChangeScaleObject.Remove(AllGameObject.SegedUpgradeTower.GetComponent<TowerOwnSolderBuilding>().UpgradeArrowTowerScriptCIrcle);
            }*/
