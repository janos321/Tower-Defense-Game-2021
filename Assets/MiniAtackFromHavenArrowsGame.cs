...
public class MiniAtackFromHavenArrowsGame : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        float a = MiniGameAllScript.APositionAttack.position.y - MiniGameAllScript.BPositionAttack.position.y;
    ...
    IEnumerator ArrowsDown(float nextTime, float downTime, int index, float present)
    {
        ...
        Transform Shadow = Instantiate(MiniGameAllScript.ShadowArrow.transform, down[index] - new Vector3(0, 2f, 0), MiniGameAllScript.attackfromheavenArrows.transform.rotation);
        //Shadow.SetParent(GameObject.Find("Enemys").transform);
        while (present <= downTime)
        {
            present += Time.deltaTime;
            Arrows[index].position = Vector3.Lerp(MiniGameAllScript.APositionAttack.position, down[index], (present) / downTime);//AllGameObject.Arrowsoblique.Evaluate(present / time));
            yield return null;
        }
        most++;
        GameObject efekt = Instantiate(MiniGameAllScript.ArrowsDOwnParticleEffect, down[index] - new Vector3(0, 2f, 0), MiniGameAllScript.attackfromheavenArrows.transform.rotation);
