...
public class location : MonoBehaviour
{
    ...
    private float TowerRangeSize = 1;

    private void Start()
    {

        TowerRangeSize = Mathf.Sqrt(Mathf.Sqrt((CameraMove.mapMaxX - CameraMove.mapMinX) * (CameraMove.mapMaxY - CameraMove.mapMinY) / 1500));
    }
    ...
    void OnMouseDown()
    {
        ...
        if (MENUGamobjects.TutorialMap == 0)
        {
            if (Mother.GetComponentInParent<TowerShot>())
            {
                ...
                Mother.GetComponent<TowerShot>().range *= TowerRangeSize;
            }
            else
                  if (Mother.GetComponentInParent<TowerOwnSolderBuilding>())
            {
                ...
                Mother.GetComponent<TowerOwnSolderBuilding>().range *= TowerRangeSize;
            }
        }
        else
        {
