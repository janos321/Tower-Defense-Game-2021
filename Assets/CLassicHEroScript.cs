...
public class CLassicHEroScript : MonoBehaviour
{
    ...
    public void Abality2Button()
    {
        ...
        if(HeroTypeINdex==1)
        {
            ...
            skill.transform.localScale = new Vector3(Circleistanse, Circleistanse, 1);
            StartCoroutine(Kepesseg());
    ...
    public IEnumerator Porges(float a)
    {
        ...
        while(Timer<a)
        {
            if ((a / DamageCount) * DamaegIndex <= Timer)
            {
                ...
                for (int i = 0; i < AllGameObject.AllEnemys.Count; i++)
                {
                    if (Mathf.Abs(Vector2.Distance(AllGameObject.AllEnemys[i].position, gameObject.transform.position)) <= Circleistanse)
                    {
    ...
    private void Update()
    {
        ...
        {
            if (Mathf.Abs(target2.x - transform.position.x) <= 0.1f || Mathf.Abs(target2.y - transform.position.y) <= 0.1f)
            {
